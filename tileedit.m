...
g_prm.pos = [x y];
prm = tilefromgui(handles);
action = getappdata(ax,'mouseaction');
update = false;
switch action
    case 'zoom_nw'
        if prm.res >= 4
...
for i=1:3
    ...
    if isempty(data.im)
        setappdata(ax(i),'mouseaction','none');
        continue;
    end
    xlim = get(ax(i),'XLim');
    ylim = get(ax(i),'YLim');
    ...
    if x >= xlim(1) && x <= xlim(2) && y >= ylim(1) && y <= ylim(2)
        ...
        switch action
            case 1 % Zoom and pan                                                                                                       
                if abs(x-xcnt) < dx/10 && y < dy/10 && prm.latidx > 0
                    marker(i) = line([xcnt-dx/20,xcnt,xcnt+dx/20],[dy/20+0.5,0.5,dy/20+0.5],'Parent',ax(i),'LineWidth',3,'Color','red','HitTest','off');
                    setappdata(ax(i),'mouseaction','panup');
                elseif abs(x-xcnt) < dx/10 && y > 9*dy/10 && prm.latidx < 2^(prm.res-4)-1
                    marker(i) = line([xcnt-dx/20,xcnt,xcnt+dx/20],[dy*19/20+0.5,dy+0.5,dy*19/20+0.5],'Parent',ax(i),'LineWidth',3,'Color','red','HitTest','off');
                    setappdata(ax(i),'mouseaction','pandown');
                elseif abs(y-ycnt) < dy/10 && x < dx/10 && prm.lngidx > 0
                    marker(i) = line([dx/20+0.5,0.5,dx/20+0.5],[ycnt-dy/20,ycnt,ycnt+dy/20],'Parent',ax(i),'LineWidth',3,'Color','red','HitTest','off');
                    setappdata(ax(i),'mouseaction','panleft');
                elseif abs(y-ycnt) < dy/10 && x > dx*9/10 && prm.lngidx < 2^(prm.res-3)-1
                    marker(i) = line([dx*19/20+0.5,dx+0.5,dx*19/20+0.5],[ycnt-dy/20,ycnt,ycnt+dy/20],'Parent',ax(i),'LineWidth',3,'Color','red','HitTest','off');
                    setappdata(ax(i),'mouseaction','panright');
                elseif abs(x-xcnt) < dx/10 && abs(y-ycnt) < dy/10 && prm.res > 1
                    marker(i) = line([xcnt-dx/20,xcnt+dx/20,nan,xcnt-dx/20,xcnt+dx/20],[ycnt-dy/20,ycnt+dy/20,nan,ycnt+dy/20,ycnt-dy/20],'Parent',ax(i),'LineWidth',3,'Color','red','HitTest','off');
                    setappdata(ax(i),'mouseaction','zoom_out');
                else
                    if prm.res < 3
                        marker(i) = rectangle('Position',[xlim(1),ylim(1),dx,dy],'Parent',ax(i),'LineWidth',3,'EdgeColor','red','HitTest','off');
                        setappdata(ax(i),'mouseaction','zoom_nw');
                    elseif prm.res == 3
                        if x < xcnt
                            marker(i) = rectangle('Position',[xlim(1),ylim(1),dx/2,dy],'Parent',ax(i),'LineWidth',3,'EdgeColor','red','HitTest','off');
                            setappdata(ax(i),'mouseaction','zoom_nw');
                        else
                            marker(i) = rectangle('Position',[xcnt,ylim(1),dx/2,dy],'Parent',ax(i),'LineWidth',3,'EdgeColor','red','HitTest','off');
                            setappdata(ax(i),'mouseaction','zoom_ne');
                        end
                    else
                        if x < xcnt
                            if y < ycnt
                                marker(i) = rectangle('Position',[xlim(1),ylim(1),dx/2,dy/2],'Parent',ax(i),'LineWidth',3,'EdgeColor','red','HitTest','off');
                                setappdata(ax(i),'mouseaction','zoom_nw');
                            else
                                marker(i) = rectangle('Position',[xlim(1),ycnt,dx/2,dy/2],'Parent',ax(i),'LineWidth',3,'EdgeColor','red','HitTest','off');
                                setappdata(ax(i),'mouseaction','zoom_sw');
                            end
                        else
                            if y < ycnt
                                marker(i) = rectangle('Position',[xcnt,ylim(1),dx/2,dy/2],'Parent',ax(i),'LineWidth',3,'EdgeColor','red','HitTest','off');
                                setappdata(ax(i),'mouseaction','zoom_ne');
                            else
                                marker(i) = rectangle('Position',[xcnt,ycnt,dx/2,dy/2],'Parent',ax(i),'LineWidth',3,'EdgeColor','red','HitTest','off');
                                setappdata(ax(i),'mouseaction','zoom_se');
                            end
                        end
                    end
                end
            ...
            case 2 % set value
                setappdata(ax(i),'mouseaction','setval');
                if g_prm.btn_down
                ...
                marker(i) = line([x-5,x-1,nan,x+1,x+5,nan,x,x,nan,x,x],[y,y,nan,y,y,nan,y-5,y-1,nan,y+1,y+5],'Parent',ax(i),'LineWidth',3,'Color','green','HitTest','off');
            case 3 % set randomized value
                setappdata(ax(i),'mouseaction','setrandom');
                if g_prm.btn_down
                    if x ~= g_prm.pos(1) || y ~= g_prm.pos(2)
                        s = get(handles.edit5,'String');
                        v = str2double(s);
            ...
            case 4 % erase
                marker(i) = line([x-5,x-1,nan,x+1,x+5,nan,x,x,nan,x,x],[y,y,nan,y,y,nan,y-5,y-1,nan,y+1,y+5],'Parent',ax(i),'LineWidth',3,'Color','green','HitTest','off');
                setappdata(ax(i),'mouseaction','erase');
            case 5 % smoothing
                marker(i) = line([x-10,x-2,nan,x+2,x+10,nan,x,x,nan,x,x],[y,y,nan,y,y,nan,y-10,y-2,nan,y+2,y+10],'Parent',ax(i),'LineWidth',3,'Color','green','HitTest','off');
                setappdata(ax(i),'mouseaction','smooth');
        end
    end
end
if mouseax==0
    ...
    if action >= 2
        for i=1:3
            if i ~= mouseax
                ...
                marker(i) = line([x-scale(2),x+scale(2),nan,x,x], [y,y,nan,y-scale(1),y+scale(1)],'Parent',ax(i),'LineWidth',1,'Color','green','HitTest','off');
